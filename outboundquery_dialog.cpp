...
#include "ui_outboundquery_dialog.h"

extern QSqlQuery sql_query;

outboundquery_Dialog::outboundquery_Dialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::outboundquery_Dialog)
{
    ui->setupUi(this);
...
outboundquery_Dialog::~outboundquery_Dialog()
{
    delete ui;
}

void outboundquery_Dialog::initSelect()
{
    QString sqlStr;
    //查询所有的用途
    sqlStr = "SELECT DISTINCT USE_TYPE FROM OUTBOUND_TABLE;";
    ...
    //查询所有的名称
    sqlStr = "SELECT DISTINCT MATERIAL_NAME FROM OUTBOUND_TABLE; ";
    ...
    //查询所有的型号
    sqlStr = "SELECT DISTINCT MATERIAL_MODEL FROM OUTBOUND_TABLE; ";
    buscess =  sql_query.exec(sqlStr);
    if(!buscess)
    {
        qDebug()<<"select error"<<endl;
    ...
    //查询所有的厂家
    sqlStr = "SELECT DISTINCT MANUFACTOR FROM OUTBOUND_TABLE; ";
    buscess =  sql_query.exec(sqlStr);
    if(!buscess)
    {
        qDebug()<<"select error"<<endl;
    ...
    //查询所有操作人
    sqlStr = "SELECT DISTINCT OPERATION_USER FROM OUTBOUND_TABLE;";
    buscess = sql_query.exec(sqlStr);
    if(!buscess)
    {
    ...
    sqlStr = "SELECT MIN(OPERATION_TIME),MAX(OPERATION_TIME) FROM OUTBOUND_TABLE;";
    buscess = sql_query.exec(sqlStr);
    if(!buscess)
    {
...
void outboundquery_Dialog::initTableWidget()
{
    ...
    for(int i=0;i<50;i++)
    {
        ...
        ui->tableWidget->setItem(i,6,&RecipientItem[i]);
        ui->tableWidget->setItem(i,7,&operationTime[i]);
        ui->tableWidget->setItem(i,8,&noteItem[i]);
        ...
        RecipientItem[i].setTextAlignment(Qt::AlignCenter);
        operationTime[i].setTextAlignment(Qt::AlignCenter);
        noteItem[i].setTextAlignment(Qt::AlignCenter);
    }
}

void outboundquery_Dialog::on_ok_pushButton_clicked()
{
    QString str[8];
    ...
    str[5] = ui->Recipient_lineEdit->text();
    str[6] = ui->dateEdit->date().toString("yyyy-MM-dd/").append(ui->dateEdit_2->date().toString("yyyy-MM-dd"));
    str[7] = ui->note_lineEdit->text();
   ...
   for(int i=0;i<8; i++)
    ...
    RecipientItem[currentItemIndex].setText(str[5]);
    operationTime[currentItemIndex].setText(str[6]);
    noteItem[currentItemIndex].setText(str[7]);
...
void outboundquery_Dialog::on_select_pushButton_clicked()
{
    QStringList sqlList;
    QString sqlStr,tmpStr;
    QString userType,materialName,materialModel,factory, operatorStr, recipient, beginTime,endTime , note;
    int num;
    ...
    for(int i=0; i<currentItemIndex; i++)
    {
        ...
        recipient = RecipientItem[i].text();
        ...
        sqlStr = "SELECT * FROM OUTBOUND_TABLE WHERE USE_TYPE='"+userType+"' and NUMBER>="+QString::number(num) + " and OPERATION_TIME>='"+beginTime + "' and OPERATION_TIME <='"+endTime+"'" ;
        if("-" != materialName)
        {
        ...
        if("-" != recipient)
        {
            tmpStr = " and RECIPIENT='"+recipient+"'";
            sqlStr.append(tmpStr);
        }
        if("-" != note)
        {
    ...
    //将此列表发送给主线程
    emit outBoundSQLResult_signal(sqlList);
