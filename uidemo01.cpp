...
{
    QString sqlStr ="SELECT * FROM PK_TABLE;";
    QStringList sqlList;
    sqlList.append(sqlStr);
    PK_selectResult_slot(sqlList);
}
...
void UIDemo01::on_PK_managerOutExcel_pushBotton_clicked()
{
    ...
    if(fileName != "")
    {
        ...
        if(excel->setControl("Excel.Application") )
        {
            ...
            int i,j;
            int colcount = ui->PK_tableWidget->columnCount();
            int rowcount = ui->PK_tableWidget->rowCount();

            QAxObject *cell,*col;
            //标题行
            cell=worksheet->querySubObject("Cells(int,int)", 1, 1);
            cell->dynamicCall("SetValue(const QString&)", "芯视界PK封装订单信息表");
            cell->querySubObject("Font")->setProperty("Size", 18);
            //调整行高
            worksheet->querySubObject("Range(const QString&)", "1:1")->setProperty("RowHeight", 30);
            //合并标题行
            ...
            for(i=0;i<colcount;i++)
            {
                ...
                cell=worksheet->querySubObject("Cells(int,int)", 2, i+1);
                //QCP_tableWidget 获取表格头部文字信息
                columnName=ui->PK_tableWidget->horizontalHeaderItem(i)->text();
                //QTableView 获取表格头部文字信息
                // columnName=ui->tableView_right->model()->headerData(i,Qt::Horizontal,Qt::DisplayRole).toString();
                cell->dynamicCall("SetValue(const QString&)", columnName);
                cell->querySubObject("Font")->setProperty("Bold", true);
            ...
            for(i=0;i<rowcount;i++){
                for (j=0;j<colcount;j++)
                {
                    worksheet->querySubObject("Cells(int,int)", i+3, j+1)->dynamicCall("SetValue(const QString&)", ui->PK_tableWidget->item(i,j)?ui->PK_tableWidget->item(i,j)->text():"");
                }
            }
            ...
            lrange.append("A2:");
            lrange.append(colcount - 1 + 'A');
            lrange.append(QString::number(ui->PK_tableWidget->rowCount() + 2));
            range = worksheet->querySubObject("Range(const QString&)", lrange);
            range->querySubObject("Borders")->setProperty("LineStyle", QString::number(1));
            range->querySubObject("Borders")->setProperty("Color", QColor(0, 0, 0));
            //调整数据区行高
            QString rowsName;
            rowsName.append("2:");
            rowsName.append(QString::number(ui->PK_tableWidget->rowCount() + 2));
            range = worksheet->querySubObject("Range(const QString&)", rowsName);
            range->setProperty("RowHeight", 20);
            workbook->dynamicCall("SaveAs(const QString&)",QDir::toNativeSeparators(fileName));//保存至fileName
            workbook->dynamicCall("Close()");//关闭工作簿
